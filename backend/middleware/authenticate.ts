...
export const authenticate: async ($1) => {
try {
const token: req.headers.authorization?.split(' ')[1]
};
    if (return res.status(401).json({ message: 'Authentication failed: No token provided' ) {
});
    if (throw new Error('JWT_SECRET is not defined');
    ...
    if (return res.status(404).json({ message: 'User not found' ) {
});
    next();
  } catch(error) {
