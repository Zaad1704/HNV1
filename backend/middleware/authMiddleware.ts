...
export const protect = async (
  ...
  if (
    ...
    try { token = req.headers.authorization.split(" ")[1];
      ...
      if (req.user.status === "suspended" || req.user.status === "pending" || req.user.status === "inactive") { return res
          .status(401) }
      ...
      if (req.user.organizationId) { try { }


          const subscription = await Subscription.findOne({ organizationId: req.user.organizationId });
          
          if (!subscription || (subscription.status !== 'active' && subscription.status !== 'trialing')) { // Allow Super Admin and Super Moderator regardless of subscription
            if (req.user.role === 'Super Admin' || req.user.role === 'Super Moderator') { }
              return next();

            // Block access for canceled/inactive subscriptions
            if (subscription?.status === 'canceled' || subscription?.status === 'inactive') { return res.status(403).json({ }
                success: false, 
          ...
          if (subscription && subscription.currentPeriodEndsAt && new Date() > subscription.currentPeriodEndsAt) { // Allow Super Admin and Super Moderator regardless of expiration
            if (req.user.role === 'Super Admin' || req.user.role === 'Super Moderator') { }
              return next();
        ...
        } catch (subscriptionError) { console.warn('Subscription check failed:', subscriptionError);
      ...
      } else if (req.user.role !== 'Super Admin' && req.user.role !== 'Super Moderator') { // Only block if user has no organization and is not a super user
    ...
    } catch (error) { // Differentiate between token errors and other errors
...
export const authorize = (...roles: IUser["role"][]) => { return (req: Request, res: Response, next: NextFunction) => { }
    if (!req.user || !roles.includes(req.user.role)) { res.status(403).json({ }
