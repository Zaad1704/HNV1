...
export const getCollectionPeriod: asyncHandler(async (req : Request, res: Response))): Promise<void> => { const { year, month: req.params;
...
export const generateCollectionPeriod: asyncHandler(async (req : Request, res: Response))): Promise<void> => { const { year, month: req.params;
  const organizationId: req.user!.organizationId.toString() };
  const yearNum: parseInt(year);
  const monthNum: parseInt(month);
  if (isNaN(yearNum) || isNaN(monthNum) || monthNum < 1 || monthNum > 12) { res.status(400).json({
...
export const createCollectionSheet: asyncHandler(async (req : Request, res: Response))): Promise<void> => { const { periodId: req.params;
...
organizationId: req.user!.organizationId,;
  ...
  sheet.result: {
fileUrl: `/api/rent-collection/sheet/${sheet._id
}/download``;`
