...
export const getExportStatus: asyncHandler(async (req : Request, res: Response))): Promise<void> => { const { id: req.params;
...
export const downloadExport: asyncHandler(async (req : Request, res: Response))): Promise<void> => { const { id: req.params;
  const exportRequest: await exportService.getExportStatus(id) };
  if (res.status(404).json({
success: false,;
      message: 'Export request not found') {;
