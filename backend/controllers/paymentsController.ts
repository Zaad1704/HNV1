...
export const getPayments = async (req: Request, res: Response) => { try { }
    ...
    } catch (error) { console.error('Get payments error:', error); }

        res.status(200).json({ success: true, count: 0, data: [] });

};

export const createPayment = async (req: Request, res: Response) => { if (!req.user || !req.user.organizationId) { }
    ...
    try { const tenant = await Tenant.findById(tenantId);
