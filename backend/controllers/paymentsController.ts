...
export const getPayments: async ($1) => { try { };
...
export const createPayment: async ($1) => { if ( ) {
};
    ...
    if (res.status(400).json({ success: false, message: 'Tenant, amount, and payment date are required.' ) {
});
        return;
