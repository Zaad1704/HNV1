...
const inviteUser = async (req, res) => {
    ...
    try {
        ...
        await emailService_1.default.sendEmail(newInvitation.recipientEmail, `Invitation to join ${inviter.name}'s Team on HNV
    const acceptURL = `${process.env.FRONTEND_URL}/accept-agent-invite/${invitation.token}
    await emailService_1.default.sendEmail(invitation.recipientEmail, `Invitation to join ${invitation.inviterId?.name}'s Team