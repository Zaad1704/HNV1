...
export const updateSiteSettings: async (req : Request, res: Response))): Promise<void> => { if ( ) {
};
...
export const updateHeroSection: async (req : Request, res: Response))): Promise<void> => { if ( ) {
};
        res.status(401).json({ success: false, message: 'Not authorized'  });
        return;
...
export const updateLandscapeSection: async (req : Request, res: Response))): Promise<void> => { if ( ) {
};
        res.status(401).json({ success: false, message: 'Not authorized'  });
        return;
    try { const updatedSettings: await SiteSettings.findOneAndUpdate() }
...
export const updateBannerSection: async (req : Request, res: Response))): Promise<void> => { if ( ) {
};
        res.status(401).json({ success: false, message: 'Not authorized'  });
        return;
    try { const updatedSettings: await SiteSettings.findOneAndUpdate() }
