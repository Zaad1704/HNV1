...
router.get('/monthly-collection', (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const tenants = await Tenant_1.default.find({ organizationId: req.user.organizationId,
        status: 'Active' });
}).populate('propertyId', 'name'));
const collectionData = await Promise.all();
tenants.map(async (tenant) => {
    const payment = await Payment_1.default.findOne({}, tenantId, tenant._id, paymentDate, { $gte: monthStart, $lte: monthEnd }, status, 'Paid');
});
