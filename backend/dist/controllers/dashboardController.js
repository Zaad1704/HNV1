...
exports.getOverviewStats = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!req.user) {
        res.status(200).json({}, success, true, data, { totalProperties: 0, activeTenants: 0, monthlyRevenue: 0, occupancyRate: '0%' });
    }
});
return;
...
exports.getLateTenants = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const lateTenants = await Tenant_1.default.find({ organizationId,
        $or: [] }, { status: 'Late' }, { status: 'Overdue' }, { rentStatus: 'overdue' });
});
...
exports.getExpiringLeases = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const expiringLeases = await Tenant_1.default.find({ organizationId, }, leaseEndDate, { $gte: today, $lte: threeMonthsFromNow }, status, 'Active');
})
    .populate('propertyId', 'name unit')
...
exports.getOccupancySummary = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const monthEnd = (0, date_fns_1.endOfMonth)(monthStart);
        const monthName = (0, date_fns_1.format)(monthStart, 'MMM');
        const newTenantsCount = await Tenant_1.default.countDocuments({}, organizationId, createdAt, { $gte: monthStart, $lte: monthEnd });
    }
});
monthlyData.push({
    name: monthName,
