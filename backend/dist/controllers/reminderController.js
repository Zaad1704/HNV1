...
const calculateNextRunDate = (currentDate, frequency) => { switch (frequency) {
} };
'daily';
...
exports.processOverdueReminders = (0, express_async_handler_1.default)(async (req, res) => {
    const now = new Date();
    const overdueReminders = await Reminder_1.default.find({}, nextRunDate, { $lte: now }, status, 'active');
...
for (const reminder of overdueReminders) {
    ...
    if (!tenant || !tenant.email) {
        console.warn(`Reminder ${reminder._id}: Tenant not found or has no email. Skipping.`, let, subject = `Rent Reminder for ${propertyName} - Unit ${tenant.unit}
      let messageBody = reminder.message || 
      `, console.error(`Error processing reminder ${reminder._id}:`, res.status(200).json({ success: true, message: `Processed ${sentCount} overdue reminders.` })));
    }
}
