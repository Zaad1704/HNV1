...
exports.deleteMaintenanceRequest = exports.getMaintenanceRequestById = exports.getOrgMaintenanceRequests = exports.createMaintenanceRequest = void 0;
    ...
    if (!req.user) {
        ...
        if (!propertyId || !description) {
            ...
            if (!property || property.organizationId.toString() !== req.user.organizationId.toString()) {
                ...
                return;
                const newRequest = await MaintenanceRequest_1.default.create({ organizationId: req.user.organizationId,
                    ...
                    requestedBy: req.user._id, }, category, notes, assignedTo, assignedTo ? new mongoose_1.Types.ObjectId(assignedTo) : undefined);
            }
            ;
            ...
            req.user._id,
                ...
                { requestId: newRequest._id.toString(), description: newRequest.description, propertyId: property._id.toString() };
        }
    }
};
                    ...
                    if (!request) {
                        ...
                        if (request.organizationId.toString() !== req.user.organizationId.toString()) {
                            ...
                            if (req.user.role === 'Tenant') {
                                ...
                                return;
                                request = await MaintenanceRequest_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true,
                                    runValidators: true, });
                            }
                            ;
                            if (request) {
                            ...
                            req.user._id,
                                ...
                                { requestId: request._id.toString(), status: request.status, description: request.description };
                        }
                    }
                }
            };
            ...
            if (request.organizationId.toString() !== req.user.organizationId.toString()) {
                ...
                if (!['Landlord', 'Agent', 'Super Admin'].includes(req.user.role)) {
                    ...
                    req.user._id,
                        ...
                        { requestId: request._id.toString(), description: request.description };
                }
            }
        }
    }
