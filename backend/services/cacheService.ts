...
class CacheService { private client: any;
  ...
  private async initializeRedis() { try { }
      if (process.env.REDIS_URL) { this.client = Redis.createClient({ }
    ...
    } catch (error) { logger.error('Failed to initialize Redis, using memory fallback:', error);
      this.setupMemoryFallback(); }
  ...
  private setupMemoryFallback() { this.isConnected = false;
    ...
    setInterval(() => { }
      const now = Date.now();
      for (const [key, item] of this.memoryCache.entries()) { if (item.expiry < now) { }
  ...
  async get(key: string): Promise<any> { try { }
      if (this.isConnected && this.client) { const value = await this.client.get(key);
    ...
    } catch (error) { logger.error('Cache get error:', error);
      return null; }



  async set(key: string, value: any, ttlSeconds = 3600): Promise<boolean> { try { }
      if (this.isConnected && this.client) { await this.client.setEx(key, ttlSeconds, JSON.stringify(value));
        return true; }
  ...
  async del(key: string): Promise<boolean> { try { }
    ...
    } catch (error) { logger.error('Cache delete error:', error);
      return false; }



  async exists(key: string): Promise<boolean> { try { }
      if (this.isConnected && this.client) { return await this.client.exists(key) === 1; }
    ...
    } catch (error) { logger.error('Cache exists error:', error);
      return false; }



  async flush(): Promise<boolean> { try { }
      if (this.isConnected && this.client) { await this.client.flushAll();
        return true; }
    ...
    } catch (error) { logger.error('Cache flush error:', error);
      return false; }
