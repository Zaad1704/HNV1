...
class RentCollectionService {
  async getCollectionSummary(organizationId: string): Promise<any> {
    try {
  ...
  async getOverdueTenants(organizationId: string): Promise<any[]> {
    try {
      ...
      console.error('Overdue tenants error:', error);
      throw error;
    }
  }

  async sendRentReminders(organizationId: string, tenantIds: string[]): Promise<any> {
    try {
      const results = {
        success: 0,
        failed: 0,
