...
class MonitoringService { private metrics: PerformanceMetrics[] = [];
  ...
  constructor() { // Collect metrics every minute
    ...
    setInterval(() => { this.cleanOldMetrics(); }
  ...
  trackRequest = (req: Request, res: Response, next: NextFunction) => { }
    ...
    res.on('finish', () => { const responseTime = Date.now() - startTime;
      ...
      // Log slow requests
      if (responseTime > 5000) { logger.warn('Slow request detected', { }
          method: req.method,
          url: req.url,
          responseTime,
  ...
  checkAlerts() { }
    ...
    if (latest.memoryUsage.percentage > 90) { alerts.push({ }
        type: 'critical',
    ...
    if (latest.errorRate > 10) { alerts.push({ }
        type: 'warning',
    ...
    if (latest.responseTime > 3000) { alerts.push({ }
        type: 'warning',
  ...
  async getDatabaseStats() { try { }
    ...
    } catch (error) { logger.error('Failed to get database stats', error);
      return null;
