...
class MasterDataService { // Initialize default system data;
  ...
  private async createDefaultPlans(): Promise<void> { const existingPlans = await Plan.countDocuments();
    if (existingPlans === 0) { }
      const defaultPlans = [
        {
  ...
  private async createDefaultSiteSettings(): Promise<void> { const existingSettings = await SiteSettings.findOne();
    if (!existingSettings) { }
      const defaultSettings = { siteName: 'HNV Property Management',
        ...
        landscapeSection: { title: 'Transform Your Property Management',
        ...
        aboutSection: { title: 'About HNV Property Management',
  ...
  private async ensureSuperAdmin(): Promise<void> {
    ...
    if (!superAdmin) { // Create super admin organization
      ...
      const adminUser = await User.create({ name: 'Super Administrator',
  ...
  async getSystemOverview(): Promise<any> { const [
      totalUsers,
      totalOrganizations,
      activeSubscriptions,
      totalRevenue,
  ...
  private async calculateTotalRevenue(): Promise<number> {
    ...
    return activeSubscriptions.reduce((total, sub: any) => { return total + (sub.planId?.price || 0); }
  ...
  async updateSiteContent(section: string, data: any): Promise<any> {
    ...
    return await SiteSettings.findOneAndUpdate({}, updateData, { new: true, 
      upsert: true; }

    });

  // Manage plans (Super Admin only)
  async createPlan(planData: any): Promise<any> { return await Plan.create(planData); }


  async updatePlan(planId: string, planData: any): Promise<any> {
    return await Plan.findByIdAndUpdate(planId, planData, { new: true });

  async deletePlan(planId: string): Promise<void> { await Plan.findByIdAndDelete(planId); }


  // Get landing page data (public)
  async getLandingPageData(): Promise<any> { const [siteSettings, publicPlans, stats] = await Promise.all([ }
  ...
  private async getPublicStats(): Promise<any> { const totalProperties = await this.getRandomStat(2500, 3000);
  ...
  private getRandomStat(min: number, max: number): number { return Math.floor(Math.random() * (max - min + 1)) + min; }
