...
router.get('/landing-data', async (req, res) => { try { }
    const landingData = await masterDataService.getLandingPageData();
    res.json({ success: true,
      data: landingData; }

    });
  } catch (error) { console.error('Landing data error:', error);
    res.json({ }
      success: true,
      data: {
...
router.get('/site-settings', async (req, res) => { try { }
  ...
  } catch (error) { console.error('Site settings error:', error);
    res.json({ }
      success: true,
      data: { siteName: 'HNV Property Management',
        contactEmail: 'support@hnvpm.com',
        siteDescription: 'Professional Property Management Solutions',
        heroTitle: 'The All-in-One Platform for Modern Property Management',
        statsTitle: 'Trusted by Property Managers Worldwide'
...
router.get('/plans', asyncHandler(async (req, res) => { try { }
  ...
  } catch (error) { console.error('Public plans error:', error);
    res.json({ }
      success: true,
      data: []
...
router.get('/validate-org-code/:code', asyncHandler(async (req, res, next) => { try { }
...
router.get('/stats', asyncHandler(async (req, res) => { try { }
  ...
  } catch (error) { console.error('Public stats error:', error);
    res.json({ }
      success: true,
      data: { totalUsers: 0,
