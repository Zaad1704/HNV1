...
const TenantDashboardPage = () => {
    ...
    if (isLoading) return <div className="text-dark-text dark:text-dark-text-dark text-center p-8">Loading Your Dashboard...</div>;
    if (isError) return (
    ...
    if (!data?.leaseInfo && !data?.paymentHistory && !data?.upcomingDues) {
        return (
            <div className="text-dark-text dark:text-dark-text-dark text-center p-8">
                ...
                <button onClick={() => setIsModalOpen(true)} className="mt-4 px-5 py-3 bg-brand-primary text-white font-bold rounded-lg hover:bg-brand-secondary transition-all duration-200">
        ...
        <div className="text-dark-text dark:text-dark-text-dark space-y-8">
            ...
            <div className="bg-gradient-to-br from-brand-primary to-brand-secondary p-6 rounded-2xl shadow-lg text-white">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    <div>
                        <p className="text-brand-subtle">Next Payment Due:</p>
                        ...
                        {upcomingDues && <p className="text-brand-subtle text-sm">Invoice: {upcomingDues.invoiceNumber}</p>}
                    </div>
                    <button
                        ...
                        className="w-full md:w-auto flex items-center justify-center gap-3 px-8 py-4 bg-white text-brand-primary font-bold rounded-lg shadow-xl hover:bg-light-card transition-all transform hover:scale-105 disabled:opacity-50 disabled:scale-100"
                    >
                        <CreditCard />
                ...
                {upcomingDues && upcomingDues.lineItems.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-brand-subtle/50">
                        <p className="text-brand-subtle text-sm font-semibold mb-2">Breakdown of Next Payment:</p>
                        <ul className="text-sm space-y-1">
                            ...
                            <li className="flex justify-between font-bold pt-1 border-t border-brand-subtle/30">
                                <span>Total</span>
            ...
            {/* Lease Info Card */}
            <div className="bg-light-card dark:bg-dark-card p-6 rounded-2xl shadow-lg border border-border-color dark:border-border-color-dark">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-light-card dark:bg-dark-card p-6 rounded-2xl shadow-lg border border-border-color dark:border-border-color-dark">
                    ...
                    {paymentHistory.length > 0 ? (
                        <div className="overflow-x-auto">
                            <table className="min-w-full text-left text-sm text-light-text dark:text-light-text-dark">
                                <thead className="text-light-text uppercase border-b border-border-color dark:border-border-color-dark dark:text-light-text-dark">
                                    <tr>
                                ...
                                <tbody>
                                    {paymentHistory.map((payment) => (
                                        <tr key={payment._id} className="border-b border-border-color last:border-b-0 dark:border-border-color-dark">
                                            ...
                                            <td className="py-2 px-3"><span className={`px-2 py-1 rounded-full text-xs font-semibold ${payment.status === 'Paid' ? 'bg-green-500/20 text-green-300' : 'bg-brand-accent-dark/20 text-brand-accent-dark'}`}>{payment.status}</span></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <div className="space-y-6">
                    <div className="bg-light-card dark:bg-dark-card p-6 rounded-2xl shadow-lg border border-border-color dark:border-border-color-dark">
                         <h2 className="text-xl font-bold text-brand-primary mb-3 flex items-center"><Wrench className="mr-2" />Submit a Maintenance Request</h2>
                    ...
                    <div className="bg-light-card dark:bg-dark-card p-6 rounded-2xl shadow-lg border border-border-color dark:border-border-color-dark">
                        ...
                        <p className="text-dark-text dark:text-dark-text-dark font-semibold flex items-center gap-2 mb-1"><Users size={18}/> {leaseInfo.landlord?.name || 'Your Landlord'}</p>
                        <p className="text-dark-text dark:text-dark-text-dark font-semibold flex items-center gap-2"><Phone size={18}/> {leaseInfo.landlord?.email || 'N/A'}</p>
                    </div>
                </div>
            </div>
        </div>
