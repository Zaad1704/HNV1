...
const PrivacyPolicyPage = () => {
  ...
  if (isLoading) return <div className="text-center p-8">Loading Privacy Policy...</div>;
  if (isError || !settings?.privacyPolicyPageContent) return <div className="text-red-500 text-center p-8">Error loading privacy policy.</div>;
    ...
    <div className="bg-light-bg min-h-screen text-dark-text">
      ...
      <main className="container mx-auto px-6 py-16">
        <div className="bg-light-card max-w-4xl mx-auto p-8 md:p-12 rounded-xl border border-border-color shadow-sm">
          <h1 className="text-4xl font-extrabold mb-6">{settings.privacyPolicyPageContent.title}</h1>
          <p className="text-sm text-light-text mb-8">Last Updated: {settings.privacyPolicyPageContent.lastUpdated}</p>
          ...
          <div className="prose prose-lg max-w-none prose-h3:text-brand-orange prose-h3:font-bold prose-a:text-brand-orange hover:prose-a:opacity-80"
               dangerouslySetInnerHTML={{ __html: settings.privacyPolicyPageContent.content }}>
          </div>
        </div>
      </main>
    </div>
