...
import { Plus, ClipboardList, Edit, Eye } from 'lucide-react';
...
// --- Edit Tenant Modal Component (No changes here, remains the same) ---
...
const TenantsPage = () => {
    ...
    const handleDownloadCollectionSheet = async () => {
        ...
        try {
            ...
            link.setAttribute('download', `Rent-Collection-Sheet-${month}-${new Date().getFullYear()}.pdf`);
            ...
            alert("Could not download the collection sheet. Please try again.");
        } finally {
                        ...
                        {tenants.map((tenant: ITenant) => (
                            <tr key={tenant._id} className="hover:bg-gray-50">
                                ...
                                <td className="p-4 text-right flex items-center justify-end gap-2">
                                    <button onClick={() => { setSelectedTenant(tenant); setIsEditModalOpen(true); }} className="font-medium text-brand-primary hover:underline flex items-center gap-1" title="Edit Tenant Details">
                                        <Edit size={16}/> Edit
                                    </button>
                                    <button onClick={() => navigate(`/dashboard/tenants/${tenant._id}/statement`)} className="font-medium text-blue-500 hover:underline flex items-center gap-1" title="View Monthly Statement">
                                        <Eye size={16}/> Statement
                                    </button>
                                </td>
                            </tr>
                        ))}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="flex items-center space-x-2 px-4 py-2.5 bg-light-card border border-border-color text-dark-text font-semibold rounded-lg hover:bg-gray-50 disabled:opacity-50"
                    >
                        <ClipboardList size={18} />
                        <span>{isDownloading ? 'Downloading...' : 'Collection Sheet'}</span>
                    </button>
                    <button
