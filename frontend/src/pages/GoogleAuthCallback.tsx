  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (error) {
        ...
        setTimeout(() => {
          navigate(`/login?error=${error}&message=${encodeURIComponent(message || 'Google authentication failed')}`);
        }, 2000);
        return;
      }
      ...
      if (token) {
        try {
          ...
          setTimeout(() => {
            navigate('/login?error=auth-verification-failed&message=Failed to verify Google authentication');
          }, 2000);
        }
      } else {
        ...
        setTimeout(() => {
          navigate('/login?error=no-token&message=No authentication token received from Google');
        }, 2000);
      }
    };
