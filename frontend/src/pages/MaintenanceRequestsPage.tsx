...
const MaintenanceRequestsPage = () => {
    ...
    const getStatusBadge = (status: string) => {
        const statusMap: { [key: string]: string } = {
            ...
            'In Progress': 'bg-brand-accent-dark/20 text-brand-accent-dark',
            'Completed': 'bg-green-500/20 text-green-300',
            'Closed': 'bg-light-text/20 text-light-text'
        };
        return statusMap[status] || 'bg-light-text/20 text-light-text';
    };
    ...
    const MobileView = () => (
        <div className="grid grid-cols-1 gap-4">
            {requests.map((req: any) => (
                <div key={req._id} className="bg-light-card p-4 rounded-xl border border-border-color shadow-sm transition-all duration-150 dark:bg-dark-card dark:border-border-color-dark">
