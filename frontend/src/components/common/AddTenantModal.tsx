            ...
            <div className="bg-light-card rounded-xl shadow-xl w-full max-w-3xl border border-border-color">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[85vh] overflow-y-auto text-light-text">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                         <div><label>Full Name*</label><input type="text" name="name" required value={formData.name} onChange={handleChange} className="mt-1 w-full bg-light-bg border-border-color p-2 rounded-md"/></div>
                         <div><label>Email*</label><input type="email" name="email" required value={formData.email} onChange={handleChange} className="mt-1 w-full bg-light-bg border-border-color p-2 rounded-md"/></div>
                         <div><label>Phone</label><input type="tel" name="phone" value={formData.phone} onChange={handleChange} className="mt-1 w-full bg-light-bg border-border-color p-2 rounded-md"/></div>
                    </div>
                    
                    <h3 className="text-lg font-semibold text-dark-text border-b border-border-color pb-2 mt-4">Lease & Property</h3>
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                           <div><label>Property*</label><select name="propertyId" required value={formData.propertyId} onChange={handleChange} disabled={isLoadingProperties} className="mt-1 w-full bg-light-bg border-border-color p-2 rounded-md"><option value="">{isLoadingProperties ? 'Loading...' : 'Select'}</option>{properties?.map((p: any) => <option key={p._id} value={p._id}>{p.name}</option>)}</select></div>
                           <div><label>Unit*</label><input type="text" name="unit" required value={formData.unit} onChange={handleChange} className="mt-1 w-full bg-light-bg border-border-color p-2 rounded-md"/></div>
                           <div><label>Monthly Rent*</label><input type="number" name="rentAmount" required value={formData.rentAmount} onChange={handleChange} className="mt-1 w-full bg-light-bg border-border-color p-2 rounded-md"/></div>
                           <div><label>Lease End Date</label><input type="date" name="leaseEndDate" value={formData.leaseEndDate} onChange={handleChange} className="mt-1 w-full bg-light-bg border-border-color p-2 rounded-md"/></div>
                       </div>
