      ...
      <div className="bg-light-card rounded-xl shadow-xl w-full max-w-lg border border-border-color" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[85vh] overflow-y-auto">
          ...
          <div>
            ...
            <input type="text" name="name" id="name" required value={formData.name} onChange={handleChange} className="mt-1 block w-full px-3 py-2 bg-dark-bg border-border-color rounded-md text-dark-text" />
          </div>
          <div>
            <label className="block text-sm font-medium text-light-text">Address</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-1">
              <input name="street" placeholder="Street" value={formData.street} onChange={handleChange} className="w-full px-3 py-2 bg-dark-bg border-border-color rounded-md text-dark-text" />
              <input name="city" placeholder="City" value={formData.city} onChange={handleChange} className="w-full px-3 py-2 bg-dark-bg border-border-color rounded-md text-dark-text" />
              <input name="state" placeholder="State" value={formData.state} onChange={handleChange} className="w-full px-3 py-2 bg-dark-bg border-border-color rounded-md text-dark-text" />
              <input name="zipCode" placeholder="Zip Code" value={formData.zipCode} onChange={handleChange} className="w-full px-3 py-2 bg-dark-bg border-border-color rounded-md text-dark-text" />
            </div>
          </div>
          <div>
            <label htmlFor="numberOfUnits" className="block text-sm font-medium text-light-text">Number of Units</label>
            <input type="number" min="1" name="numberOfUnits" id="numberOfUnits" required value={formData.numberOfUnits} onChange={handleChange} className="mt-1 block w-full px-3 py-2 bg-dark-bg border-border-color rounded-md text-dark-text"/>
          </div>
          <div>
            <label htmlFor="image" className="block text-sm font-medium text-light-text">Property Image</label>
            <input type="file" name="image" id="image" onChange={handleFileChange} className="mt-1 block w-full text-sm text-light-text file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-light-text file:bg-dark-bg hover:file:bg-border-color"/>
          </div>
          ...
          <div className="flex justify-end space-x-4 pt-4">
              ...
              <button type="submit" disabled={mutation.isLoading} className="px-5 py-2 bg-brand-primary text-dark-text font-semibold rounded-lg hover:opacity-90 disabled:opacity-50 flex items-center gap-2">
