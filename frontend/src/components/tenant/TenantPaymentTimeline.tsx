...
const TenantPaymentTimeline: React.FC<TenantPaymentTimelineProps> = ({ tenantId, className = '' }) => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'late': return <XCircle size={16} className="text-red-500" />;
  ...
  if (!timeline.length) {
    return (
      <div className={`bg-gray-50 rounded-2xl p-6 text-center ${className}`}>
        ...
        <p className="text-gray-600">No payment timeline available</p>
      </div>
    );
  }
    ...
    <div className={`bg-white rounded-2xl border border-gray-100 ${className}`}>
      <div className="p-6 border-b border-gray-100">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <div className="text-lg font-bold text-gray-900">${totalExpected.toLocaleString()}</div>
          ...
          <div>
            <div className="text-lg font-bold text-red-600">${(totalExpected - totalPaid).toLocaleString()}</div>
      ...
      <div className="p-6">
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {timeline.map((month, index) => (
            <div 
              key={index}
              className={`flex items-center gap-4 p-4 rounded-xl border-l-4 border-l-${getStatusColor(month.status)}-500 bg-${getStatusColor(month.status)}-50 hover:bg-${getStatusColor(month.status)}-100 transition-colors`}
            >
                ...
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-4">
                    <span className="text-gray-600">
                      Expected: <span className="font-medium">${month.expected.toLocaleString()}</span>
                    </span>
                    <span className={`text-${getStatusColor(month.status)}-600`}>
              ...
              <div className="flex-shrink-0 text-right">
                <div className={`text-lg font-bold text-${getStatusColor(month.status)}-600`}>
